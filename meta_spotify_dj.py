...
logging.basicConfig(
    filename="meta_spotify_dj.log", encoding="utf-8", level=logging.DEBUG, filemode="w"
)
logging.info(
...
while running:
    ...
    if menu_choice == "2":
        ...
        while running:
            spotify_manager.auth_manager, spotify_manager.spotify_connection = (
                spotify_manager.Refresh_Spotify(
                    spotify_manager.auth_manager, spotify_manager.spotify_connection
                )
            )
            ...
            if screen_image is not None:
                ...
                for request in current_requests:
                    buffer_search = next(
                        (
                            x
                            for x in requests_buffer
                            if x.requestor == request.requestor
                            and x.track == request.track
                        ),
                        None,
                    )
                    if buffer_search is None:
                        ...
                        results = spotify_manager.spotify_connection.search(
                            request.track, 1, 0, "track"
                        )
                        songs_dict = results["tracks"]
                        song_items = songs_dict["items"]
                        song = song_items[0]["external_urls"]["spotify"]
                        spotify_manager.spotify_connection.add_to_queue(
                            song, spotify_manager.spotify_device_id
                        )
