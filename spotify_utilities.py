...
class SpotifyManager:
    ...
    def __init__(self):
        with open("dj_config.json", "r") as config_file:
            ...
            self.client_id = os.environ.get('SPOTIFY_CLIENT_ID') if not None else spotify_config.get("client_id")
            self.client_secret = os.environ.get('SPOTIFY_CLIENT_SECRET') if not None else spotify_config.get("client_secret")
    ...
    def GetAndSetSpotifyDevice(self):
        ...
        try:
            ...
            print("You do not have an active Spotify client running.")
            exit()
        return device_id
